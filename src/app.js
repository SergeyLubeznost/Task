...
<div id="contSelect" class="hidden">
 ...
 <div id="contSelect" class="hidden">
  ...
  if (savedtasksFinish) {
    if (login === "admin") {
      if (JSON.parse(savedtasksFinish) && JSON.parse(savedtasksFinish).length) {
        tasksFinish = JSON.parse(savedtasksFinish);
      }

      console.log("tasksFinish", tasksFinish);
    ...
    `;
        ...
        removeButton.addEventListener("click", () => {
          ...
          if (tasksFinish && tasksFinish.length) {
  ...
  submitToGoInFinish.addEventListener("click", () => {
    ...
    const selectedTaskFinish = tasksInProgress.splice(
      selectedTaskIndexFinish,
      1
    )[0]; // Удаляем выбранный элемент из массива
    ...
    const selectedTicketBlock =
      inProgressColumn.childNodes[selectedTaskIndexFinish];
  ...
  `;
   ...
   // if (login === "admin") {
      ...
      removeButton.addEventListener("click", () => {
        ...
        for (
          let i = selectedTaskIndexFinish;
          i < selectTicketFinich.options.length;
          i++
        ) {
          selectTicketFinich.options[i].value =
            parseInt(selectTicketFinich.options[i].value) - 1;
        }
  ...
  inProgressColumn.addEventListener("drop", (event) => {
    ...
    for (let i = selectedTaskIndex; i < selectTicket.options.length; i++) {
      selectTicket.options[i].value =
        parseInt(selectTicket.options[i].value) - 1;
    }
  ...
  submitToGo.addEventListener("click", () => {
    ...
    console.log("savedtasksFinish", savedtasksFinish);
    console.log("JSON.parse(savedtasksFinish)", JSON.parse(savedtasksFinish));
